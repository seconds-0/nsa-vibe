...
def check_data_fetch_health(hb_records: List[Dict]) -> Tuple[bool, str]:
    ...
    if len(slow_fetches) > len(fetch_times) * 0.1:  # More than 10% slow
        ...
        return False, f"Data fetch stalls: {len(slow_fetches)}/{len(fetch_times)} slow (avg {avg_slow:.2f}s)"
...
def extract_metrics(csv_records: List[Dict], hb_records: List[Dict]) -> Dict:
    ...
    if gate_records:
        ...
        gate_collapse = sum(r.get('gate_collapse_frac', 0.0) for r in recent_gates) / len(recent_gates)
    
    return {
        'final_loss': final_loss,
        'first_100_loss_avg': first_100_avg,
        'last_100_loss_avg': last_100_avg,
        'loss_improvement_pct': (first_100_avg - last_100_avg) / first_100_avg * 100 if first_100_avg > 0 else 0.0,
...
def compare_with_baseline(current: Dict, baseline: Dict, tolerance_pct: float = 5.0) -> Tuple[bool, List[str]]:
    ...
    for metric, direction, description in checks:
        ...
        if direction == 'lower' and change_pct < -tolerance_pct:
            issues.append(f"{description}: {curr_val:.4f} vs baseline {base_val:.4f} ({change_pct:+.1f}%)")
...
def run_synthetic_smoke(steps: int = 1000, timeout: int = 300) -> bool:
    ...
    cmd = [
        sys.executable, "-u", "scripts/train_showcase.py",
        "--dataset", "synthetic",
        "--ddp", "0",
        f"--steps", str(steps),
    ]
...
def main() -> int:
    ...
    parser.add_argument("--csv", type=str, default="artifacts/train_showcase/training.csv",
                       help="Path to training CSV")
    parser.add_argument("--heartbeat", type=str, default="artifacts/train_showcase/heartbeat_rank0.jsonl",
                       help="Path to heartbeat JSONL")
    parser.add_argument("--min-steps", type=int, default=100,
                       help="Minimum steps required for validation")
    parser.add_argument("--min-tps", type=float, default=10.0,
                       help="Minimum tokens/second threshold")
    parser.add_argument("--run-synthetic", action="store_true",
                       help="Run synthetic data smoke test")
    parser.add_argument("--run-fineweb", action="store_true", 
                       help="Run FineWeb-Edu smoke test")
    parser.add_argument("--smoke-steps", type=int, default=1000,
                       help="Steps to run in smoke tests")
    parser.add_argument("--smoke-timeout", type=int, default=600,
                       help="Timeout for smoke tests (seconds)")
    parser.add_argument("--save-baseline", type=str,
                       help="Save current metrics as baseline to specified file")
    parser.add_argument("--compare-baseline", type=str,
                       help="Compare current metrics against baseline file")
    parser.add_argument("--baseline-tolerance", type=float, default=5.0,
    ...
    if csv_path.exists():
        ...
        print(f"[smoke] Found {len(csv_records)} training steps, {len(hb_records)} heartbeat records")
        ...
        if args.compare_baseline:
            if baseline_passed:
                checks.append(("Baseline comparison", (True, f"Within {args.baseline_tolerance}% tolerance")))
            else:
