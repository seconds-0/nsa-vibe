...
def main():
    ...
    it = fineweb_stream_batches(
        ...
        report_docs=args.report_docs,
    )
    print(
        f"[smoke] pulling first batch: S={args.seq_len} B={args.batch} tokenizer={args.tokenizer}"
    )
    box = {}
    import threading
    ...
    if not box.get("ok"):
        print(
            f"[smoke][FAIL] loader error or timeout ({args.timeout}s): {box.get('err', 'timeout')}"
        )
        raise SystemExit(2)
