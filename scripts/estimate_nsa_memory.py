...
def estimate_nsa_memory(dim, n_layers, n_heads, seq_len, batch_size=1, dtype_bytes=2):
    ...
    attention_memory = (
        batch_size * n_layers * n_heads * seq_len * seq_len * dtype_bytes / (1024 * 1024)
    )
...
"""
    ...
    if config["seq_len"] >= 1024 and config["dim"] >= 768:
        report += (
            "⚠️  WARNING: Large dim + long sequence detected. Expect quadratic memory growth!\n"
        )
...
def main():
    ...
    parser.add_argument("--dtype", choices=["bf16", "fp32"], default="bf16", help="Data type")
    parser.add_argument(
        "--compare-configs", action="store_true", help="Compare common configurations"
    )
    ...
    if args.compare_configs:
        ...
        configs = [
            ...
            {
                "name": "Production (12L, dim=768, S=2048)",
                "dim": 768,
                "n_layers": 12,
                "n_heads": 12,
                "seq_len": 2048,
        ...
        for cfg in configs:
            ...
            print(
                f"{cfg['name']:<35} {est['forward_total_gb']:>10.2f} GB {est['backward_total_gb']:>10.2f} GB  {status}"
            )
