...
def main() -> int:
    ...
    with profile(
        ...
        cmd = [sys.executable, '-u', 'scripts/train_showcase.py', '--dataset', args.dataset, '--steps', str(args.steps)] + extra
        env = os.environ.copy()
    ...
    try:
        ...
        with profile(activities=activities, record_shapes=True, with_stack=False, profile_memory=True) as prof2:
            pass
        # Use the earlier prof object for key_averages if available
        tbl = prof.key_averages().table(sort_by='self_cuda_time_total' if torch.cuda.is_available() else 'self_cpu_time_total', row_limit=200)
