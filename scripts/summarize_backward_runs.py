...
def generate_summary_report(results_dir, output_file=None):
    ...
    if scaling_tests:
        ...
        for test in sorted(scaling_tests, key=lambda x: x.get("config", {}).get("seq_len", 0)):
            ...
            report.append(f"| {test['name']} | {seq_len} | {status_emoji} {status} | {forward_mb} | {backward_mb} |")
    
    # Branch analysis
    report.append(f"\n## Branch Analysis")
    
    branch_tests = [r for r in all_results if any(b in r.get("name", "").lower() for b in ["win", "sel", "cmp"])]
    if branch_tests:
    ...
    if scaling_tests:
        seq_128 = next((t for t in scaling_tests if t.get("config", {}).get("seq_len") == 128), None)
        seq_512 = next((t for t in scaling_tests if t.get("config", {}).get("seq_len") == 512), None)
        ...
        if seq_128 and seq_512:
            ...
            if mem_128 and mem_512:
                ...
                if scaling_factor > 8:  # 4x sequence should be ~16x memory for O(S²)
                    ...
                    report.append(f"- Scaling factor: {scaling_factor:.1f}x (expected 4x for linear)")
