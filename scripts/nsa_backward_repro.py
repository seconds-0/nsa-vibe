...
def run_test(args):
    ...
    if args.profile:
        ...
        with profile(
            ...
            print(
                f"After forward: {mem_after_forward['allocated_mb']:.1f} MB allocated, {mem_after_forward['reserved_mb']:.1f} MB reserved"
            )
        ...
        try:
            ...
            print(
                f"After backward: {mem_after_backward['allocated_mb']:.1f} MB allocated, {mem_after_backward['reserved_mb']:.1f} MB reserved"
            )
...
def main():
    ...
    parser.add_argument(
        "--branch", choices=["win", "sel", "cmp"], help="Force single branch (win/sel/cmp)"
    )
    parser.add_argument("--sel", choices=["masked", "packed", "gather"], help="Selection backend")
    parser.add_argument("--cmp", choices=["masked", "parity"], help="Compressed backend")
    parser.add_argument("--win", choices=["masked", "parity"], help="Sliding window backend")

    # Debugging options
    parser.add_argument("--anomaly", action="store_true", help="Enable autograd anomaly detection")
    parser.add_argument("--blocking", action="store_true", help="Enable CUDA_LAUNCH_BLOCKING")
    parser.add_argument("--profile", action="store_true", help="Enable PyTorch profiler")
    parser.add_argument("--hooks", action="store_true", help="Register backward hooks")
