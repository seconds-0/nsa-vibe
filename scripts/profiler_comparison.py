...
def run_benchmark(
    name: str, env_vars: Dict[str, str], steps: int, warmup: int, out_dir: Path
) -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument(
        "--warmup", type=int, default=10, help="Warmup steps to exclude from metrics"
    )
    parser.add_argument(
        "--out", type=str, default="artifacts/profiler_comparison", help="Output directory"
    )
    ...
    configs = [
        {
            "name": "baseline_v1",
            "env": {"NSA_SEL_RANGES_V2": "0", "NSA_DDP_COMPRESS": "none", "NSA_NVTX": "0"},
        },
        {
            "name": "v2_ranges",
            "env": {"NSA_SEL_RANGES_V2": "1", "NSA_DDP_COMPRESS": "none", "NSA_NVTX": "0"},
        },
        {
            "name": "v2_ranges_nvtx",
            "env": {"NSA_SEL_RANGES_V2": "1", "NSA_DDP_COMPRESS": "none", "NSA_NVTX": "1"},
        },
    ]
            ...
            [
                {
                    ...
                    "env": {"NSA_SEL_RANGES_V2": "1", "NSA_DDP_COMPRESS": "bf16", "NSA_NVTX": "0"},
                },
                {
                    "name": "v2_ranges_ddp_fp16",
                    "env": {"NSA_SEL_RANGES_V2": "1", "NSA_DDP_COMPRESS": "fp16", "NSA_NVTX": "0"},
                },
            ]
        )
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            "| Configuration | Avg Throughput (toks/s) | Speedup | Peak Memory (MiB) | Final Loss |\n"
        )
        f.write(
            "|---------------|-------------------------|---------|-------------------|------------|\n"
        )
        ...
        v1_result = next(
            (r for r in results if r["name"] == "baseline_v1" and not r.get("error")), None
        )
        v2_result = next(
            (r for r in results if r["name"] == "v2_ranges" and not r.get("error")), None
        )
