...
def run_sliding_benchmark(heads, dk, dv, S_list, w, iters, device):
    ...
    for S in S_list:
        ...
        if device.type == "cuda" and fa2_supported(device, Q.dtype, dk):
            s_ref, s_new = bench_pair(sliding_window_attention_masked, sliding_window_attention_fa2, Q, K, V, w, iters=iters)
            speedup = s_ref / max(s_new, 1e-9)
            print(f"S={S} w={w} sliding masked {s_ref*1e3:.2f} ms  fa2 {s_new*1e3:.2f} ms  speedup x{speedup:.2f}")
        else:
...
def run_compressed_benchmark(heads, dk, dv, S_list, l, d, iters, device):
    ...
    for S in S_list:
        ...
        if device.type == "cuda" and fa2_supported(device, Q.dtype, dk):
            c_ref, c_new = bench_pair(batched_causal_attention_compressed_masked, compressed_attention_fa2, Q, K_cmp, V_cmp, l, d, iters=iters)
            speedup = c_ref / max(c_new, 1e-9)
            print(f"S={S} l={l} d={d} compressed masked {c_ref*1e3:.2f} ms  fa2 {c_new*1e3:.2f} ms  speedup x{speedup:.2f}")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["win", "cmp", "legacy"], default="legacy", 
    ...
    parser.add_argument("--S_list", type=str, default="128,256,512,1024,2048", 
    ...
    if args.mode == "win":
        ...
        run_compressed_benchmark(args.heads, args.dk, args.dv, S_list, args.l, args.d, args.iters, device)
    else:
