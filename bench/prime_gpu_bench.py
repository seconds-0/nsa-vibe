...
class PrimeIntellectBenchmark:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        if not self.api_key:
            raise ValueError(
                "PRIME_API_KEY not found. Set environment variable or pass api_key parameter."
            )
    ...
    def find_cheapest_gpu(self, gpu_type: str, regions: list[str] | None = None) -> dict:
        ...
        resp = requests.get(
            f"{self.base_url}/availability/", params=params, headers=self.headers, timeout=30
        )
    ...
    def create_benchmark_pod(
        ...
        for attempt in range(3):
            try:
                resp = requests.post(
                    f"{self.base_url}/pods/", json=pod_config, headers=self.headers, timeout=30
                )
                ...
                if resp.status_code in [200, 201]:  # 201 = Created
                    ...
                    print(
                        f"Created pod: {self.pod_id} (status: {pod_data.get('status', 'unknown')})"
                    )
                    return self.pod_id
    ...
    def wait_for_pod(self, pod_id: str, timeout: int = 300) -> dict:
        ...
        while time.time() - start < timeout:
            resp = requests.get(f"{self.base_url}/pods/{pod_id}", headers=self.headers)
    ...
    def determine_thresholds(
        self, results: list[BenchmarkResult], safety_margin: float = 1.1
    ) -> tuple[int, int]:
        """
    ...
    def cleanup(self) -> None:
        ...
        if self.pod_id:
            ...
            try:
                resp = requests.delete(f"{self.base_url}/pods/{self.pod_id}", headers=self.headers)
                if resp.status_code == 200:
                    result = resp.json()
    ...
    def run_full_benchmark(
        self, gpu_type: str = "RTX4090_24GB", safety_margin: float = 1.2
    ) -> dict:
        """
...
""",
    ...
    parser.add_argument(
        "--gpu-type", default="RTX4090_24GB", help="GPU type to use for benchmarking"
    )
    ...
    parser.add_argument("--max-price", type=float, help="Maximum hourly price willing to pay")

    parser.add_argument("--api-key", help="Prime Intellect API key (or set PRIME_API_KEY env var)")
    ...
    try:
        results = benchmark.run_full_benchmark(
            gpu_type=args.gpu_type, safety_margin=args.safety_margin
        )
        ...
        for r in results["results"]:
            if r["branch"] == "sliding":
                ...
                print(f"  S={r['sequence_length']:4} (compressed): speedup {r['speedup']:.2f}x")
