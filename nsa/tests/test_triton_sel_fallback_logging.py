...
@pytest.mark.skipif(
    SM89_BLOCK and not FORCED,
    reason="Triton disabled by ADR on SM 8.9; set NSA_TRITON_SEL_FORCE=1 for experiments",
)
...
def test_group_path_smoke():
    # Set environment variables and track originals for cleanup
    env_vars = {"NSA_USE_TRITON_SEL": "1", "NSA_SEL_TRITON_GROUP": "1", "NSA_SEL_TRITON_MIN_L": "1"}
    original_values = _set_env_vars(env_vars)
    ...
    try:
        ...
        V = torch.randn(B, G, S_kv, Dv, device=device, dtype=torch.float16)
        ranges = (
            torch.tensor([[[[[0, 40], [64, 96]]]]], device=device, dtype=torch.int64)
            .expand(B, S, G, 2, 2)
            .contiguous()
        )
