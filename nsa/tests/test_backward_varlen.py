...
from nsa.core.attention_kernels import sliding_window_attention_fa2, batched_causal_attention_compressed_masked, compressed_attention_fa2, sliding_window_attention_masked


RUN_GPU = os.getenv("NSA_TEST_TRAIN", "0").lower() in ("1", "true", "yes") and torch.cuda.is_available()


@pytest.mark.skipif(not RUN_GPU, reason="GPU opt-in tests; set NSA_TEST_TRAIN=1 and ensure CUDA available")
...
def test_backward_parity_compressed_gpu():
	...
	S_cmp = (S - l) // d + 1
	K_cmp = torch.stack([K_raw[:, :, i * d : i * d + l].mean(dim=2) for i in range(S_cmp)], dim=2).detach().requires_grad_(True)
	V_cmp = torch.stack([V_raw[:, :, i * d : i * d + l].mean(dim=2) for i in range(S_cmp)], dim=2).detach().requires_grad_(True)
	...
	loss_fa2 = O_fa2.pow(2).mean()
	loss_fa2.backward()
	mae = sum((a - b).abs().mean().item() for a, b in zip(grads_ref, (Q.grad, K_cmp.grad, V_cmp.grad))) / 3.0
	assert mae <= 2e-4
