...
class TestFA2Parity:
    ...
    def test_random_inputs_parity(self):
        ...
        correlation = torch.corrcoef(torch.stack([out_sdpa.flatten(), out_fa2.flatten()]))[
            0, 1
        ].item()
    ...
    def test_uniform_input_with_correct_layouts(self):
        ...
        assert abs(sdpa_val - fa2_val) < 0.1, (
            f"Outputs differ: SDPA={sdpa_val:.4f}, FA-2={fa2_val:.4f}"
        )
