...
RUN_GC = os.getenv("NSA_TEST_TRAIN", "0").lower() in ("1", "true", "yes") and torch.cuda.is_available()


@pytest.mark.skipif(not RUN_GC, reason="Gradcheck (GPU opt-in); set NSA_TEST_TRAIN=1 and ensure CUDA")
