...
def _run_needle_check(S_ctx: int, device: torch.device) -> None:
    ...
    meta = build_block_meta(S_ctx, l=DEFAULT_L, d=DEFAULT_D, l_sel=DEFAULT_L_SEL, 
                           n_sel=DEFAULT_N_SEL, w=DEFAULT_W)
    ...
    p_cmp_all = torch.zeros((DEFAULT_B, DEFAULT_SQ, DEFAULT_G, DEFAULT_H, S_cmp), 
    ...
    t_token = S_ctx - 1
    ranges = select_topn_ranges(p_grp, meta, n_top=DEFAULT_N_SEL, t_token=t_token, 
                               force_init=True, force_local=2)
...
def test_selection_mapping_includes_needle_64k_cuda():
    ...
    if torch.cuda.is_available():
        ...
        if total_memory < required_memory:
            pytest.skip(f"Insufficient GPU memory for 64k test: {total_memory / 1024**3:.1f}GB < {required_memory / 1024**3:.1f}GB")
