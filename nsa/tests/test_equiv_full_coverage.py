...
def full_attn_ref_from_branch_weights(x: torch.Tensor, W_Q: nn.Linear, W_K: nn.Linear, W_V: nn.Linear, W_O: nn.Linear, n_heads: int, n_kv_groups: int, d_k: int, d_v: int) -> torch.Tensor:
    B, S, _ = x.shape
...
def test_selection_full_coverage_equiv():
    ...
    nsa = NSAAttention(dim=dim, n_heads=n_heads, n_kv_groups=G, d_k=d_k, d_v=d_v, l=1, d=1, l_sel=1, n_sel=S, w=0)
    ...
    y_ref = full_attn_ref_from_branch_weights(x, nsa.W_Q, nsa.W_K_sel, nsa.W_V_sel, nsa.out, n_heads, G, d_k, d_v)
...
def test_compressed_full_coverage_equiv():
    ...
    n_heads, G, d_k, d_v = 1, 1, 16, 16
    nsa = NSAAttention(dim=dim, n_heads=n_heads, n_kv_groups=G, d_k=d_k, d_v=d_v, l=1, d=1, l_sel=1, n_sel=1, w=0)
    x = torch.randn(B, S, dim)
    ...
    nsa.gate.fc2.bias.data = torch.tensor([1000.0, -1000.0, -1000.0])
    y_nsa, _ = nsa(x, kv, prefill=True)
    y_ref = full_attn_ref_from_branch_weights(x, nsa.W_Q, nsa.W_K_cmp, nsa.W_V_cmp, nsa.out, n_heads, G, d_k, d_v)
    mae = (y_nsa - y_ref).abs().mean().item()
    assert mae < 1e-5
